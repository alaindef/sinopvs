...
bool CFSData::CreateSocket()
{
	...
	getsockopt(ListeningSock,SOL_SOCKET,SO_RCVBUF,(char*)&buff_size,&len);                         // teut
	...
	if(setsockopt(ListeningSock,SOL_SOCKET,SO_BROADCAST,&broadcast,sizeof(broadcast)) < 0)
        cout << "broadcast problem" << endl;
    if (setsockopt(ListeningSock,SOL_SOCKET, SO_REUSEADDR,&reuseaddr,sizeof(reuseaddr)) < 0)
        cout << "REUSEADDR problem" << endl;
    if (setsockopt(ListeningSock,SOL_SOCKET,SO_RCVBUF,(char*)&buff_size,len))
        cout << "RCVBUF problem" << endl;
    int errorcode = errno;                                          // teut errorcode=22
